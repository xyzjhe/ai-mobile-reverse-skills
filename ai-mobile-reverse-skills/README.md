...
- **Fact**：各 Phase 写入的关键发现，带可信度级别（PATTERN / CONFIRMED / EXPLOITED）
- **Intent**：上游 Phase 对下游 Phase 的定向信号（如 Phase 2 指定 Phase 3 的 native 分析目标）
- **Hint**：总控在 auto_chain 模式下写入的质量警告（如上游 Phase 未产出新 Fact 时的 stall_warning）

正常使用时，测试人员不需要手工维护这两个文件。  
`analysis_state.json` 由总控更新；`session_blackboard.json` 由各 Phase Agent 写入，所有下游 Phase 可读。
...
auto_chain 模式下，总控在每个 Phase 完成后检查新增 Fact 数；若为 0，向 blackboard 写 `stall_warning` Hint 给下一 Phase，提示上游可能覆盖不足。
